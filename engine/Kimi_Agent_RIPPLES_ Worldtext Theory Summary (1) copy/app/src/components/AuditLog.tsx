...
import type { AuditEntry, VectorType, Ripple, CrossScenarioRipple, EmergentPattern } from '@/types';
...
export function AuditLog({ entries, tick, ripples, crossScenarioRipples, emergentPatterns }: AuditLogProps) {
    ...
    <div className="w-80 h-full bg-card/50 border-l border-border flex flex-col">
      ...
      <div className="p-3 border-b border-border">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="text-[10px] text-muted-foreground">
            Ripples: <span className="font-mono text-foreground">{ripples.length}</span>
          </span>
        </div>
      </div>
            ...
            entries.map((entry, index) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {entry.propagationDepth > 0 && (
                    <span className="text-[9px] text-orange-400">
                      +{entry.affectedEntities.length} prop
                    </span>
                  )}
                ...
                {entry.affectedEntities.length > 0 && (
                  <div className="mt-1 pt-1 border-t border-border/30 flex flex-wrap gap-1">
                    {entry.affectedEntities.map((name, i) => (
                      <span key={i} className="text-[9px] text-orange-400">
                        → {name}
                      </span>
                    ))}
                  </div>
      ...
      {/* Cross-scenario ripples */}
      {crossScenarioRipples.length > 0 && (
        <div className="p-3 border-t border-border">
          ...
          <div className="space-y-1">
            {crossScenarioRipples.slice(-3).map((ripple, i) => (
              <div key={i} className="text-[9px] text-muted-foreground">
                <span className={vectorColors[ripple.vector]}>{ripple.vector}</span>
                {' '}from <span className="text-foreground">{ripple.fromScenario}</span>
                {' '}→ <span className="text-foreground">{ripple.toScenario}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {emergentPatterns.length > 0 && (
        <div className="p-3 border-t border-border">
          ...
          <div className="space-y-1">
            {emergentPatterns.slice(0, 2).map((pattern) => (
              <div key={pattern.id} className="text-[9px]">
                <div className="text-foreground/80">{pattern.name}</div>
      ...
      <div className="p-3 border-t border-border">
        ...
        <div className="mt-2 pt-2 border-t border-border/50">
          <div className="flex items-center justify-between text-[9px] text-muted-foreground">
            <span>Propagated ripples:</span>
            <span className="text-foreground font-mono">
              {entries.filter(e => e.propagationDepth > 0).length}
            </span>
          </div>
          <div className="flex items-center justify-between text-[9px] text-muted-foreground">
            <span>Total affected entities:</span>
            <span className="text-foreground font-mono">
              {entries.reduce((sum, e) => sum + e.affectedEntities.length, 0)}
            </span>
          </div>
        </div>
      </div>
